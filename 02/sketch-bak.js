...
// Line mode
let useBezierMode = false; // Toggle between straight line and bezier curve
...
function draw() {
  ...
  if (activePrinciple !== null) {
    ...
    if (animationProgress >= 1.0) {
      // Loop animation instead of stopping
...
function drawAnimatedLine() {
  ...
  if (activePrinciple === null) {
    ...
    if (useBezierMode) {
      ...
      line(baseX, baseY - lineHeight / 2, baseX, baseY + lineHeight / 2);
    }
  } else {
    // Apply principle-specific animation
    if (useBezierMode) {
      applyBezierPrincipleAnimation(activePrinciple, animationProgress, baseX, baseY, lineHeight);
    } else {
      applyPrincipleAnimation(activePrinciple, animationProgress, baseX, baseY, lineHeight);
    }
  }
}
...
function applyPrincipleAnimation(principle, progress, baseX, baseY, lineHeight) {
  // Apply smooth loop easing to create seamless transitions
  const smoothProgress = smoothLoop(progress);
  
  // Each principle demonstrates a different animation concept
  
  switch(principle) {
    case 0: // Squash & Stretch
      animateSquashStretch(smoothProgress, baseX, baseY, lineHeight);
      break;
    case 1: // Anticipation
      animateAnticipation(smoothProgress, baseX, baseY, lineHeight);
      break;
    case 2: // Staging
      animateStaging(smoothProgress, baseX, baseY, lineHeight);
      break;
    case 3: // Straight Ahead & Pose to Pose
      animateStraightAhead(smoothProgress, baseX, baseY, lineHeight);
      break;
    case 4: // Follow Through
      animateFollowThrough(smoothProgress, baseX, baseY, lineHeight);
      break;
    case 5: // Slow In & Slow Out
      animateSlowInOut(smoothProgress, baseX, baseY, lineHeight);
      break;
    case 6: // Arc
      animateArc(smoothProgress, baseX, baseY, lineHeight);
      break;
    case 7: // Secondary Action
      animateSecondaryAction(smoothProgress, baseX, baseY, lineHeight);
      break;
    case 8: // Timing
      animateTiming(smoothProgress, baseX, baseY, lineHeight);
      break;
    case 9: // Exaggeration
      animateExaggeration(smoothProgress, baseX, baseY, lineHeight);
      break;
    case 10: // Solid Drawing
      animateSolidDrawing(smoothProgress, baseX, baseY, lineHeight);
      break;
    case 11: // Appeal
      animateAppeal(smoothProgress, baseX, baseY, lineHeight);
      break;
  }
}

// Principle 0: Squash & Stretch
function animateSquashStretch(t, x, y, h) {
  // Line bounces and squashes/stretches
  const bounceHeight = 150;
  const phases = 4; // number of bounces
  
  // Ease for the bounce
  const bounceT = sin(t * PI * phases);
  const yOffset = -abs(bounceT) * bounceHeight;
  
  // Squash when at bottom, stretch when moving
  const velocity = abs(cos(t * PI * phases));
  const squash = map(velocity, 0, 1, 0.5, 1.5);
...
function animateAnticipation(t, x, y, h) {
  // Line moves back before moving forward
  let xOffset = 0;
  
  if (t < 0.2) {
    // Anticipation - move back
    xOffset = map(t, 0, 0.2, 0, -50);
  } else {
    // Main action - move forward with ease
    const actionT = map(t, 0.2, 1, 0, 1);
    const eased = easeOutCubic(actionT);
    xOffset = map(eased, 0, 1, -50, 200);
  }
  
  line(x + xOffset, y - h / 2, x + xOffset, y + h / 2);
}

// Principle 2: Staging
function animateStaging(t, x, y, h) {
  // Line moves from dark to spotlight
  const spotX = map(t, 0, 1, -WIDTH / 4, WIDTH / 4);
  
  // Draw spotlight effect
  noStroke();
  fill(255, 255, 255, 20);
  circle(x + spotX, y, 200);
  
  // Draw line in the spotlight
  stroke('#ffffff');
  strokeWeight(LINE_WIDTH);
  line(x + spotX, y - h / 2, x + spotX, y + h / 2);
}

// Principle 3: Straight Ahead & Pose to Pose
function animateStraightAhead(t, x, y, h) {
  // Line draws itself from bottom to top
  const drawProgress = easeInOutCubic(t);
  const currentHeight = h * drawProgress;
  
  line(x, y + h / 2 - currentHeight, x, y + h / 2);
}

// Principle 4: Follow Through
function animateFollowThrough(t, x, y, h) {
  // Line stops but top continues moving (overlapping action)
  const stopPoint = 0.6;
  let xOffset, rotation;
  
  if (t < stopPoint) {
    xOffset = map(t, 0, stopPoint, 0, 150);
    rotation = 0;
  } else {
    xOffset = 150;
    const followT = map(t, stopPoint, 1, 0, 1);
    rotation = sin(followT * PI * 2) * 0.3 * (1 - followT);
  }
...
// Principle 5: Slow In & Slow Out (Ease)
function animateSlowInOut(t, x, y, h) {
  // Line moves with easing
  const eased = easeInOutCubic(t);
  const xOffset = map(eased, 0, 1, -150, 150);
  
  line(x + xOffset, y - h / 2, x + xOffset, y + h / 2);
}

// Principle 6: Arc
function animateArc(t, x, y, h) {
  // Line follows an arc path
  const angle = map(t, 0, 1, -PI / 2, PI / 2);
  const radius = 150;
  const xOffset = sin(angle) * radius;
  const yOffset = -cos(angle) * radius + radius;
...
// Principle 7: Secondary Action
function animateSecondaryAction(t, x, y, h) {
  // Main action: line moves right
  const xOffset = map(t, 0, 1, 0, 150);
  
  // Secondary action: line rotates
  const rotation = sin(t * PI * 4) * 0.2;
...
// Principle 8: Timing
function animateTiming(t, x, y, h) {
  // Show fast vs slow timing - two lines moving at different speeds
  const fastT = min(t * 2, 1);
  const slowT = t * 0.5;
  
  const fastX = map(fastT, 0, 1, 0, 150);
  const slowX = map(slowT, 0, 1, 0, 150);
  
  // Fast line (thinner)
  strokeWeight(LINE_WIDTH / 2);
  line(x - 50 + fastX, y - h / 2, x - 50 + fastX, y + h / 2);
  
  // Slow line (original)
  strokeWeight(LINE_WIDTH);
  line(x + 50 + slowX, y - h / 2, x + 50 + slowX, y + h / 2);
}

// Principle 9: Exaggeration
function animateExaggeration(t, x, y, h) {
  // Exaggerated squash and stretch with overshoot
  const bounceT = t < 0.5 ? t * 2 : (1 - t) * 2;
  const overshoot = sin(bounceT * PI);
  
  const scaleX = 1 + overshoot * 2;
...
// Principle 10: Solid Drawing
function animateSolidDrawing(t, x, y, h) {
  // Line rotates in 3D space (simulated with perspective)
  const angle = map(t, 0, 1, 0, TWO_PI);
  const scale3D = abs(cos(angle));
...
// Principle 11: Appeal
function animateAppeal(t, x, y, h) {
  // Smooth, appealing motion with bounce
  const eased = easeOutElastic(t);
  const xOffset = map(eased, 0, 1, -100, 100);
...
function animateBezierSquashStretch(t, x, y, h) {
  // Tunable parameters
  const bounceCount = 4;
  const bounceHeight = 180;
  const maxSquashAmount = 120;
  const controlPointSpread = 80;
  
  const bounceT = sin(t * PI * bounceCount);
  const yOffset = -abs(bounceT) * bounceHeight;
  const velocity = abs(cos(t * PI * bounceCount));
  
  // Squash when hitting bottom, stretch when in air
  const squash = map(velocity, 0, 1, maxSquashAmount, -maxSquashAmount * 0.5);
  
  // Anchor points
  const x1 = x;
  const y1 = y - h / 2 + yOffset + squash;
  const x2 = x;
  const y2 = y + h / 2 + yOffset - squash;
  
  // Control points bulge out when squashed
  const bulge = map(velocity, 0, 1, controlPointSpread, 0);
  const cx1 = x - bulge;
