...
function draw() {
  ...
  const motifExtent = shapeSize + (numShapes - 1) * spacing; // total size of one motif
  ...
  for (let i = 0; i < copies; i++) {
    push();
    translate(width * 0.5, startY + i * step);
    scale(stackScales[i]);
    // Apply DIFFERENCE to all inner copies (exclude first/last)
    if (i > 0 && i < copies - 1) {
